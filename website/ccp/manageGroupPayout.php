...
define("SELECTED_ROWS_FIELD_NAME", "groupPayoutIds");
define("HIDDEN_ROW_FIELD_NAME", "rowGroupPayoutId");
define("DEFAULT_VALUE_GROUP_ID", "-1");
define("DEFAULT_VALUE_PAYOUT_ID", "-1");
define("DEFAULT_VALUE_TEMP_GROUP_PAYOUT_ID", "");
define("DEFAULT_VALUE_GROUP_PAYOUT_NAME", "");
define("DEFAULT_VALUE_PAYOUT_NAME", "");
$smarty->assign("title", "Chip Chair and a Prayer Manage Group Payout");
$smarty->assign("script", "<script src=\"scripts/manageGroupPayout.js\" type=\"text/javascript\"></script>\n");
...
$smarty->assign("formName", "frmManageGroupPayout");
print_r($_POST);
$groupPayoutIds = isset($_POST[SELECTED_ROWS_FIELD_NAME]) ? $_POST[SELECTED_ROWS_FIELD_NAME] : DEFAULT_VALUE_TEMP_GROUP_PAYOUT_ID;
$aryGroupPayoutIds = explode("::", $groupPayoutIds);
...
if (Constant::$MODE_CREATE == $mode || Constant::$MODE_MODIFY == $mode) {
  ...
  if (Constant::$MODE_CREATE == $mode || (Constant::$MODE_MODIFY == $mode && $groupPayoutIds != DEFAULT_VALUE_TEMP_GROUP_PAYOUT_ID)) {
    $ctr = 0;
    $ary = explode(Constant::$DELIMITER_DEFAULT, $groupPayoutIds);
    while ($ctr < count($ary)) {
      $arySplit = explode("::", $ary[$ctr]);
  ...
  $hiddenSelectedRows = new FormControl(SessionUtility::getValue(SessionUtility::$OBJECT_NAME_DEBUG), null, null, false, null, null, null, false, SELECTED_ROWS_FIELD_NAME, null, SELECTED_ROWS_FIELD_NAME, null, null, false, null, null, null, null, FormControl::$TYPE_INPUT_HIDDEN, $groupPayoutIds, null);
  ...
  $groupPayoutIds = DEFAULT_VALUE_TEMP_GROUP_PAYOUT_ID;
  $mode = Constant::$MODE_VIEW;
}
if (Constant::$MODE_VIEW == $mode || Constant::$MODE_DELETE == $mode || Constant::$MODE_CONFIRM == $mode) {
  if (Constant::$MODE_CONFIRM == $mode) {
    if (DEFAULT_VALUE_TEMP_GROUP_PAYOUT_ID != $groupPayoutIds) {
...
//   $link = array(array(3), array("manageUser.php", array("userId", "mode"), 2, "modify", 3));
  ...
  $htmlTable = new HtmlTable(null, null, null, null, SessionUtility::getValue(SessionUtility::$OBJECT_NAME_DEBUG), Constant::$DELIMITER_DEFAULT, null, true, null, HIDDEN_ROW_FIELD_NAME, null, null, null, $link, true, $query, $groupPayoutIds, null, "30%");
