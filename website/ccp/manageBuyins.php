...
if (Constant::$MODE_SAVE_CREATE == $mode || Constant::$MODE_SAVE_MODIFY == $mode) {
  ...
  for ($index = 0; $index < count($aryPlayers); $index ++) {
    if (" " != $aryFees[$index]) {
      $params = array($aryFees[$index],SessionUtility::getValue(SessionUtility::$OBJECT_NAME_ID),$aryPlayers[$index],$tournamentId);
      $rowCount = $databaseResult->updateFees(params: $params);
      if (0 == $rowCount) {
        $params = array(SessionUtility::getValue(SessionUtility::$OBJECT_NAME_ID),$aryPlayers[$index]);
        $rowCount = $databaseResult->deleteFeeBySeasonAndPlayer(params: $params);
        $params = array(SessionUtility::getValue(SessionUtility::$OBJECT_NAME_ID),$aryPlayers[$index],$tournamentId,$aryFees[$index]);
...
if ($mode == Constant::$MODE_VIEW) {
  ...
  if ($tournamentId != DEFAULT_VALUE_TOURNAMENT_ID) {
    ...
    // $checkboxFeeButton = new FormControl(debug: SessionUtility::getValue(SessionUtility::$OBJECT_NAME_DEBUG), accessKey: null, autoComplete: null, autoFocus: false, checked: null, class: null, cols: null, disabled: false, id: Constant::$TEXT_FEE . "_?2", maxLength: null, name: Constant::$TEXT_FEE . "_?2", import: $import, onClick: "input.changeState({checkId: '" . Base::build(Constant::$TEXT_FEE, null) . "_?2', aryChangeId: new Array(" . (0 == $maxRebuys ? "''" : "'" . Base::build(Constant::$TEXT_REBUY, null) . "_?2'") . (0 == $addonAmount ? "" : ", '" . Base::build(Constant::$TEXT_ADDON, null) . "_?2'") . ", '', '')}); document.querySelector('#" . HIDDEN_ROW_FEE_PAID_FIELD_NAME . "').value = '?3';", placeholder: null, readOnly: false, required: null, rows: null, size: null, suffix: "_?2", type: FormControl::$TYPE_INPUT_CHECKBOX, value: "?1", wrap: null);
